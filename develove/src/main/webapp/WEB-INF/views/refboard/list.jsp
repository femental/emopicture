	...
	<section class="content">
		<div style="width:70%;margin-left:15%;">
			<table class="table tabel-bordered">
				...
				<c:forEach var="refboardVO" items="${list}">
				
					<tr>
						<td>${refboardVO.bno}</td>
						<td><a href="${refboardVO.bno}">${refboardVO.title}[${refboardVO.replycnt}]</a></td>
						<td>${refboardVO.nickname}</td>
						<td>${refboardVO.regdate}</td>
						<td>${refboardVO.viewcnt}</td>
					</tr>
				</c:forEach>
...
$(document).ready(function () {
    ...
    $(".table tr td a").on("click",function(event){
        ...
        criform.attr("action","/refboard/view").submit(); 
    });
    ...
    $("#newBtn").on("click", function(){
    	var criform = $("#cri");
    	criform.attr("action","/refboard/create");
    	criform.submit();
    });
