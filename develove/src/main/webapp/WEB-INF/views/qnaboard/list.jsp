	...
	<section class="content">
		<div style="width:70%;margin-left:15%;">
			<table class="table tabel-bordered">
				...
				<c:forEach var="qnaboardVO" items="${list}">
				
					<tr>
						<td>${qnaboardVO.bno}</td>
						<td><a href="${qnaboardVO.bno}">${qnaboardVO.title} [${qnaboardVO.replycnt}]</a></td>
						<td>${qnaboardVO.nickname}</td>
						<td>${qnaboardVO.regdate}</td>
						<td>${qnaboardVO.viewcnt}</td>
						<td>${qnaboardVO.answeryn}</td>
					</tr>
				</c:forEach>
...
<!-- /.content-wrapper -->
	<!-- cridata -->
	<form id="cri">
		<input type="hidden" name="pageNo" value="${cri.pageNo}"> <input
			type="hidden" name="searchType" value="${cri.searchType}"> <input
			type="hidden" name="keyword" value="${cri.keyword}"> <input
			type="hidden" name="totalCount" value="${cri.totalCount}"> <input
	...
	$(document).ready(
			function() {
				...
				$(".table tr td a").on("click", function(event) {
					...
					criform.attr("action", "/qnaboard/view").submit();
				});
